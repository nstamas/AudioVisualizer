...
void VisualizationEngine::Update(const AudioData& audioData, float deltaTime) {
    ...
    // Smooth the spectrum for better visuals
    for (size_t i = 0; i < std::min(audioData.spectrum.size(), smoothedSpectrum_.size()); ++i) {
        smoothedSpectrum_[i] = smoothedSpectrum_[i] * 0.7f + audioData.spectrum[i] * 0.3f;
    }
    
    // Update rotation for animated visualizations
    rotation_ += deltaTime * 20.0f;  // 20 degrees per second
    ...
    if (audioData.beat > 0.5f) {
        if (timeSinceLastBeat_ > 0.2f) {  // Throttle particle spawning
...
void VisualizationEngine::RenderBars() {
    ...
    const float maxHeight = static_cast<float>(height_) * 2.0f;
...
void VisualizationEngine::RenderCircularSpectrum() {
    ...
    float baseRadius = std::min(width_, height_) * 0.2f;
    float maxLength = std::min(width_, height_) * 0.4f;
    
    float angleStep = 360.0f / numBars;
    
    for (size_t i = 0; i < numBars; ++i) {
        float magnitude = smoothedSpectrum_[i];
        float barLength = magnitude * maxLength;
...
void VisualizationEngine::RenderRadialBars() {
    ...
    float centerY = height_ * 0.5f;
    float angleStep = 360.0f / numBars;
    float maxLength = std::min(width_, height_) * 0.45f;
    
    for (size_t i = 0; i < numBars; ++i) {
        float magnitude = smoothedSpectrum_[i];
        float barLength = magnitude * maxLength;
        float angle1 = (rotation_ + i * angleStep) * 3.14159f / 180.0f;
        float angle2 = (rotation_ + (i + 1) * angleStep) * 3.14159f / 180.0f;
...
void VisualizationEngine::RenderMirroredBars() {
    ...
    const float barWidth = static_cast<float>(width_) / numBars;
    const float maxHeight = static_cast<float>(height_) * 0.45f;
...
void VisualizationEngine::CreateShaderPresets() {
    ...
    std::string plasmaShader = R"(
        ...
        void main() {
            ...
            float v2 = sin(uv.y + time * 0.5);
            float v3 = sin(uv.x + uv.y + time * 0.3);
            float v4 = sin(sqrt(uv.x*uv.x + uv.y*uv.y) + time);
            float v = (v1 + v2 + v3 + v4) * 0.25;
            
            vec3 col = vec3(
                sin(v * 3.14159 + bass * 2.0) * 0.5 + 0.5,
                sin(v * 3.14159 + mid * 2.0 + 2.0) * 0.5 + 0.5,
                sin(v * 3.14159 + treble * 2.0 + 4.0) * 0.5 + 0.5
            );
    ...
    std::string tunnelShader = R"(
        ...
        void main() {
            ...
            float tunnel = 1.0 / (r + 0.1) + time * (1.0 + bass * 2.0);
            float spiral = a / 3.14159 + time * 0.5;
            
            vec3 col = vec3(
                sin(tunnel * 5.0 + bass * 3.0) * 0.5 + 0.5,
                sin(spiral * 8.0 + mid * 3.0) * 0.5 + 0.5,
                sin((tunnel + spiral) * 3.0 + treble * 3.0) * 0.5 + 0.5
            );
    ...
    std::string fractalShader = R"(
        ...
        void main() {
            vec2 uv = (fragCoord - 0.5) * 3.0;
            uv.x *= resolution.x / resolution.y;
            
            vec2 z = uv;
            vec2 c = vec2(sin(time * 0.3) * 0.5, cos(time * 0.2) * 0.5) * (1.0 + bass);
            ...
            float t = iter * 0.1 + time;
            vec3 col = vec3(
                sin(t + bass * 2.0) * 0.5 + 0.5,
                sin(t * 0.7 + mid * 2.0) * 0.5 + 0.5,
                sin(t * 0.3 + treble * 2.0) * 0.5 + 0.5
            );
            
            fragColor = vec4(col * (iter / 50.0), 1.0);
        }
    )";
    ...
    std::string kaleidoscopeShader = R"(
        ...
        void main() {
            ...
            float segments = 6.0 + bass * 6.0;
            a = mod(a, 3.14159 * 2.0 / segments);
            a = abs(a - 3.14159 / segments);
            
            vec2 p = vec2(cos(a), sin(a)) * r;
            float pattern = sin(p.x * 10.0 + time) * sin(p.y * 10.0 + time * 0.7);
            
            vec3 col = vec3(
                sin(pattern * 5.0 + bass * 2.0) * 0.5 + 0.5,
                sin(pattern * 7.0 + mid * 2.0) * 0.5 + 0.5,
                sin(pattern * 11.0 + treble * 2.0) * 0.5 + 0.5
            );
    ...
    std::string waveShader = R"(
        ...
        void main() {
            ...
            float wave1 = sin(uv.x * 10.0 + time * 2.0 + bass * 5.0) * 0.1;
            float wave2 = sin(uv.y * 8.0 + time * 1.5 + mid * 5.0) * 0.1;
            float wave3 = sin((uv.x + uv.y) * 6.0 + time + treble * 5.0) * 0.1;
            
            float d = abs(uv.y - 0.5 - wave1 - wave2 - wave3);
            float line = smoothstep(0.02, 0.0, d);
            
            vec3 col = vec3(
                line * (sin(time + bass * 2.0) * 0.5 + 0.5),
                line * (sin(time * 0.7 + mid * 2.0) * 0.5 + 0.5),
                line * (sin(time * 0.3 + treble * 2.0) * 0.5 + 0.5)
            );
...
void VisualizationEngine::RenderShaderVisualization() {
    ...
    // Calculate frequency bands for shader uniforms
    ...
    shaderManager_->SetFloat(shaderId, "bass", bass * 10.0f);
    shaderManager_->SetFloat(shaderId, "mid", mid * 10.0f);
    shaderManager_->SetFloat(shaderId, "treble", treble * 10.0f);
